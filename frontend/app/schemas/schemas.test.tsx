...
test('Test schema with invalid statuses', async () => {
    ...
    expect(() => taskSchema.parse({ title: 'do something', status: 'todo', label: 'something', priority: 'low' })).toThrow("Invalid label");
    expect(() => taskSchema.parse({ title: 'do something', status: 'todo', label: 'epic', priority: 'standar' })).toThrow("Invalid priority");
});